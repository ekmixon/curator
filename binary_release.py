...
if platform == 'win32':
    ...
    build_name = f'exe.{sys_string}-{pyver}'
    target_name = f"curator-{str(get_version())}-{sys_string}"
else:
...
if os.path.exists(build_path):
    ...
    if os.path.exists(target_path):
        ...
        if float(pyver) >= 2.7:
            ...
            if platform == 'win32':
                fname = f'elasticsearch-{target_name}.zip'
            else:
                fname = f'elasticsearch-{target_name}.tar.gz'
