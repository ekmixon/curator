...
def _age_elements(action, config):
    ...
    retval = [filter_elements.source(action=action, required=is_req)]
    ...
    if (
        ...
        and action in settings.index_actions()
        and config['source'] == 'field_stats'
    ):
